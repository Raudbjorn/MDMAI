...
class AdaptiveLearningSystem:
    ...
    def _extract_content_features(self, text: str) -> Dict[str, Any]:
        ...
        features = {
            ...
            "has_spell_keywords": any(kw in text.lower() for kw in [
                'casting time', 'components', 'duration', 'spell'
            ]),
            "has_monster_keywords": any(kw in text.lower() for kw in [
                'hit points', 'armor class', 'challenge', 'creature'
            ]),
            "has_rule_keywords": any(kw in text.lower() for kw in [
                'must', 'cannot', 'action', 'bonus action', 'reaction'
            ]),
        }
    ...
    def _apply_corrections(self, corrections: Dict[str, Any], system: str):
        ...
        for content_type, examples in corrections.items():
            ...
            for example in examples:
                self.patterns[system][content_type].add_example(
                    example["text"],
                    example["fields"]
                )
