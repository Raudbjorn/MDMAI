...
"""
    ...
    async def _save_alert(self, alert: Alert) -> None:
        ...
        try:
            alerts_file = self.storage_path / f"alerts_{datetime.now().strftime('%Y%m')}.json"
            
            # Load existing alerts or create new file using async file I/O
            if alerts_file.exists():
                content = await asyncio.to_thread(alerts_file.read_text)
            ...
            # Save back to file using async I/O
            content = json.dumps(data, indent=2)
            await asyncio.to_thread(alerts_file.write_text, content)
