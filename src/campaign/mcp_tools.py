...
def initialize_campaign_tools(db: ChromaDBManager, cm: CampaignManager, rl: RulebookLinker) -> None:
    """
...
async def create_campaign(
    name: str, system: str, description: str = "", setting: str = ""
) -> Dict[str, Any]:
    """
    ...
    if not campaign_manager:
        return {"success": False, "message": "Campaign manager not initialized"}

    try:
        result = await campaign_manager.create_campaign(
            name=name, system=system, description=description, setting=setting
        )
...
async def get_campaign_data(campaign_id: str, data_type: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        return await campaign_manager.get_campaign_data(
            campaign_id=campaign_id, data_type=data_type
        )

    except Exception as e:
        logger.error(f"Failed to get campaign data: {str(e)}")
        return {"success": False, "message": f"Failed to get campaign data: {str(e)}"}


async def update_campaign_data(
    campaign_id: str, data_type: str, data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        if data_type == "character":
            ...
            result = await campaign_manager.update_campaign(
                campaign_id, data, data.get("change_description", "Campaign updated")
            )
        else:
        ...
        return {"success": False, "message": f"Failed to update campaign data: {str(e)}"}


async def list_campaigns(system: Optional[str] = None, limit: int = 100) -> Dict[str, Any]:
    """
    ...
    try:
        campaigns = await campaign_manager.list_campaigns(system=system, limit=limit)

        return {"success": True, "campaigns": campaigns, "count": len(campaigns)}

    except Exception as e:
        logger.error(f"Failed to list campaigns: {str(e)}")
        return {"success": False, "message": f"Failed to list campaigns: {str(e)}"}
...
async def get_campaign_versions(campaign_id: str, limit: int = 10) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"success": False, "message": f"Failed to get campaign versions: {str(e)}"}


async def rollback_campaign(campaign_id: str, version_number: int) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"success": False, "message": f"Failed to rollback campaign: {str(e)}"}
...
async def set_active_campaign(campaign_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "message": f"Failed to set active campaign: {str(e)}"}


async def get_campaign_references(
    campaign_id: str, entity_type: Optional[str] = None, entity_id: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    if not rulebook_linker:
        return {"success": False, "message": "Rulebook linker not initialized"}
    ...
    try:
        ...
        return {"success": False, "message": f"Failed to get campaign references: {str(e)}"}
...
async def validate_campaign_references(campaign_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "message": f"Failed to validate references: {str(e)}"}


async def search_with_campaign_context(
    query: str, campaign_id: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    if not campaign_manager or not rulebook_linker:
        return {"success": False, "message": "Campaign system not initialized"}
    ...
    try:
        ...
        if not campaign_id:
            return {"success": False, "message": "No campaign specified or active"}

        # Get campaign context
        context = await rulebook_linker.get_campaign_context_for_search(
            campaign_id=campaign_id, query=query
        )
        ...
        return {"success": False, "message": f"Failed to search with context: {str(e)}"}
