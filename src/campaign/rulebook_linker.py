...
class RulebookLinker:
    ...
    def _extract_potential_references(self, text: str) -> List[Tuple[str, float]]:
        ...
        return [(text, conf) for text, conf in 
                zip(seen.keys(), seen.values())][:10]  # Limit to top 10
    ...
    async def _store_references(
        self,
        campaign_id: str,
        references: Dict[str, List[RulebookReference]]
    ) -> None:
        """
        ...
        try:
            ...
            if doc:
                ...
                if cache_key in ref_data:
                    refs = [
                        RulebookReference(**ref_dict)
                        for ref_dict in ref_data[cache_key]
                    ]
        ...
        try:
            ...
            for entity_key, refs in ref_data.items():
                for ref_dict in refs:
                    ...
                    rulebook_doc = self.db_manager.get_document(
                        "rulebooks",
                        ref.rulebook_id
                    )
    ...
    @handle_search_errors()
    async def get_campaign_context_for_search(
        self,
        campaign_id: str,
        query: str
    ) -> Dict[str, Any]:
