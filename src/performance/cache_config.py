...
class CacheProfile:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CacheProfile":
        ...
        try:
            ...
            logger.warning(
                f"Invalid cache_type '{data.get('cache_type')}' found in config. Defaulting to GENERAL."
            )
...
class CacheConfiguration:
    ...
    def get_profiles_by_type(self, cache_type: CacheType) -> List[CacheProfile]:
        ...
        """
        return [p for p in self.profiles.values() if p.cache_type == cache_type]
    ...
    def get_cache_recommendations(self) -> Dict[str, Any]:
        ...
        for profile in self.profiles.values():
            ...
            if profile.cache_type == CacheType.EMBEDDING and profile.ttl_seconds < 86400:
                profile_recs.append("Consider increasing TTL for embeddings (expensive to compute)")
            elif profile.cache_type == CacheType.SESSION and profile.ttl_seconds > 7200:
                profile_recs.append("Consider reducing TTL for sessions (temporary data)")

            # Check persistence
            if (
                profile.cache_type in [CacheType.EMBEDDING, CacheType.SOURCE]
                and not profile.persistent
            ):
                profile_recs.append("Consider enabling persistence for expensive-to-compute data")

            # Check policy
            if profile.cache_type == CacheType.SEARCH and profile.policy != "lru":
                profile_recs.append("LRU policy recommended for search cache")
        ...
        recommendations["memory"] = {
            ...
            "utilization": (
                f"{(total_memory / settings.cache_max_memory_mb * 100):.1f}%"
