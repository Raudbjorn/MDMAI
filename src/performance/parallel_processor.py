...
class ResourceLimits:
    ...
    max_process_workers: int = field(default_factory=lambda: max(1, mp.cpu_count() // 2))
...
class ParallelProcessor:
        ...
        async with self._task_lock:
            ...
            with self._executor_lock:
                ...
                self.executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=self.limits.max_workers, thread_name_prefix="ttrpg-worker"
                )
                ...
                self.process_executor = concurrent.futures.ProcessPoolExecutor(
                    max_workers=self.limits.max_process_workers, mp_context=ctx
                )
            ...
            logger.info(f"Parallel processor initialized with {self.limits.max_workers} workers")
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(
                        self._process_task(task, worker_id), timeout=self.limits.task_timeout
                    )
    ...
    async def _process_pdf_task(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            parser = PDFParser()
            pdf_content = await loop.run_in_executor(self.executor, parser.parse_pdf, str(pdf_path))
    ...
    async def _process_embedding_task(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            batch_embeddings = await asyncio.gather(
                *[self._generate_single_embedding(generator, chunk) for chunk in batch]
            )
    ...
    async def _generate_single_embedding(self, generator, chunk: str) -> List[float]:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(self.executor, generator.generate_embedding, chunk)
    ...
    async def _process_batch_task(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i in range(0, len(operations), batch_size):
            ...
            batch_results = await asyncio.gather(
                *[self._execute_operation(op) for op in batch], return_exceptions=True
            )
    ...
    async def _resource_monitor(self) -> None:
        ...
        while not self._shutdown:
            try:
                ...
                logger.error(
                    f"Resource monitor error ({consecutive_errors}/{max_consecutive_errors}): {str(e)}"
                )
    ...
    async def submit_task(
        ...
        # Validate task type
        allowed_task_types = {"pdf_processing", "embedding_generation", "search", "batch_operation"}
    ...
    async def process_multiple_pdfs(
        ...
        try:
            ...
            for pdf_info in pdf_files:
                task = await self.processor.submit_task("pdf_processing", pdf_info)
                pdf_tasks.append(task)

            # Wait for PDF processing to complete
            completed_tasks = await self.processor.wait_for_all([t.id for t in pdf_tasks])
                ...
                for task in completed_tasks:
                    if task.status == TaskStatus.COMPLETED and task.result:
                        emb_task = await self.processor.submit_task(
                            "embedding_generation", {"chunks": task.result.get("content", [])}
                        )
                        embedding_tasks.append(emb_task)

                # Wait for embeddings
                await self.processor.wait_for_all([t.id for t in embedding_tasks])
            ...
            return {
                ...
                "failed": stats["failed"],
                "statistics": stats,
                "results": [t.to_dict() for t in completed_tasks],
            }
...
class ResourceManager:
    ...
    def allocate_processor(
        ...
        logger.info(f"Allocated processor {processor_id} with {limits.max_workers} workers")
