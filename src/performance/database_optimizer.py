...
class DatabaseOptimizer:
    ...
    async def optimize_indices(self, collection_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        collections_to_optimize = (
            ...
            else list(self.db.collections.keys())
        )
    ...
    async def _optimize_collection_index(self, collection_name: str) -> IndexMetrics:
        ...
        try:
            ...
            if hasattr(collection, "_compact"):
                await asyncio.get_event_loop().run_in_executor(
                    self.executor, collection._compact
                )
        except Exception as e:
        ...
        return IndexMetrics(
            ...
            optimization_count=self.index_metrics.get(collection_name, IndexMetrics(
                collection_name, 0, 0, 0, 0
    ...
    def _analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        if analysis["length"] > 500:
            analysis["suggestions"].append("Query is very long - consider breaking into multiple searches")
        
        if analysis["word_count"] > 20:
            analysis["suggestions"].append("Many words in query - consider focusing on key terms")
        
        if analysis["has_special_chars"]:
            analysis["suggestions"].append("Special characters may slow search - use simple terms when possible")
