...
async def get_cache_stats(
    ...
    try:
        ...
        if cache_name:
            ...
            if cache_name in _cache_manager.caches:
                ...
                if detailed:
                    ...
                    cache_stats["invalidation_history"] = (
                        _cache_invalidator.get_invalidation_stats() if _cache_invalidator else {}
                    )
            ...
            if detailed:
                total_hits = sum(c.stats.total_hits for c in _cache_manager.caches.values())
                total_misses = sum(c.stats.total_misses for c in _cache_manager.caches.values())
                total_memory = sum(c._get_total_size() for c in _cache_manager.caches.values())
...
async def configure_cache(
    ...
    try:
        ...
        if max_size is not None:
            if max_size <= 0:
                return {"success": False, "error": "max_size must be positive"}
            profile.max_size = max_size
            updated.append("max_size")
        if max_memory_mb is not None:
            if max_memory_mb <= 0:
                return {"success": False, "error": "max_memory_mb must be positive"}
            profile.max_memory_mb = max_memory_mb
            updated.append("max_memory_mb")
        if ttl_seconds is not None:
            if ttl_seconds < 0:
                return {"success": False, "error": "ttl_seconds cannot be negative"}
    ...
    try:
        ...
        if total_memory_mb is None:
            total_memory_mb = _cache_config.get_global_setting(
                "max_total_memory_mb", 100  # Default fallback
            )
