...
class SearchService:
    ...
    def _explain_result(self, result: SearchResult, processed_query: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if result.keyword_score > 0:
            ...
            matching_terms = self._find_matching_terms(
                result.content, 
        ...
        # Generate summary
        primary_factor = max(
            explanation["relevance_factors"],
            key=lambda x: x.get("score", 0.5)
        ) if explanation["relevance_factors"] else None
    ...
    def _track_search(self, query: str, result_count: int, search_time: float, from_cache: bool = False):
        """
        ...
        try:
            ...
            if campaign_results and campaign_results.get("documents"):
                for i, doc in enumerate(campaign_results["documents"][0]):
                    metadata = campaign_results["metadatas"][0][i] if campaign_results.get("metadatas") else {}
        ...
        try:
            ...
            if session_results and session_results.get("documents"):
                for i, doc in enumerate(session_results["documents"][0]):
                    metadata = session_results["metadatas"][0][i] if session_results.get("metadatas") else {}
    ...
    async def _find_cross_references(
        ...
        try:
            ...
            if entities.get("monsters"):
                cross_refs["related_monsters"] = await self._find_related_monsters(entities["monsters"])
    ...
    async def _extract_campaign_entities(self, campaign_id: str, query: str) -> List[Dict[str, Any]]:
