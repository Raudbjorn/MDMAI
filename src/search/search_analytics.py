...
class SearchMetrics:
    ...
    def add_search(self, latency: float, relevance_score: float, result_count: int, from_cache: bool = False):
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_results": sum(self.result_counts) / len(self.result_counts) if self.result_counts else 0,
            "cache_hit_rate": self.cache_hits / (self.cache_hits + self.cache_misses) if (self.cache_hits + self.cache_misses) > 0 else 0,
            "empty_result_rate": self.empty_results / self.query_count if self.query_count > 0 else 0,
...
class SearchAnalytics:
    ...
    def track_search(
        ...
        with self.lock:
            ...
            self.hourly_metrics[hour_key].add_search(latency, avg_relevance, result_count, from_cache)
    ...
    def get_performance_metrics(self, time_range: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            summary["relevance_distribution"] = self._get_relevance_distribution(metrics.relevance_scores)
            summary["result_count_distribution"] = self._get_result_distribution(metrics.result_counts)
    ...
    def get_trend_analysis(self, metric: str = "volume", period: str = "hour") -> List[Dict[str, Any]]:
        """
    ...
    def get_cache_analytics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                "cache_effectiveness": time_saved / self.current_metrics.total_latency if self.current_metrics.total_latency > 0 else 0,
    ...
    def get_error_analysis(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                "error_rate": self.current_metrics.errors / self.current_metrics.query_count if self.current_metrics.query_count > 0 else 0,
    ...
    def _get_optimization_recommendations(self) -> List[str]:
        ...
        if summary["empty_result_rate"] > 0.2:
            recommendations.append("High empty result rate - consider improving query processing or content coverage")
        ...
        if len(self.slow_queries) > 10:
            recommendations.append(f"Found {len(self.slow_queries)} slow queries - consider query optimization")
