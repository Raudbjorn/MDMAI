...
class CharacterGenerator:
    ...
    def generate_character(
        ...
        character = Character(
            system=system,
            name=name or self._generate_name(race)
        )
        ...
        # Generate equipment
        character.equipment = self._generate_equipment(
            character.character_class,
            level
        )
        ...
        # Generate features
        character.features = self._generate_class_features(
            character.character_class,
            level
        )
        ...
        if self._is_spellcaster(character.character_class):
            character.spells = self._generate_spells(
                character.character_class,
                level
            )
        ...
        character.backstory = self._generate_basic_backstory(
            character,
            backstory_hints
        )
    ...
    def _generate_stats(
        ...
        if method == "standard":
            ...
            primary_stats = self.CLASS_PRIMARY_STATS.get(
                character_class,
                ["strength", "dexterity"]
            )
    ...
    def _generate_class_features(
        ...
        if level >= 5:
            features.append("Extra Attack" if character_class in [
    ...
    def _is_spellcaster(self, character_class: CharacterClass) -> bool:
        ...
        spellcasters = [
            CharacterClass.WIZARD, CharacterClass.CLERIC,
            CharacterClass.SORCERER, CharacterClass.WARLOCK,
            CharacterClass.DRUID, CharacterClass.BARD,
            CharacterClass.PALADIN, CharacterClass.RANGER,
            CharacterClass.ARTIFICER
        ]
        return character_class in spellcasters
    
    def _generate_spells(
        self,
        character_class: CharacterClass,
        level: int
    ) -> List[str]:
