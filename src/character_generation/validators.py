...
class CharacterValidator:
    ...
    def validate_stats(cls, stats: CharacterStats) -> List[str]:
        ...
        if not cls.MIN_LEVEL <= stats.level <= cls.MAX_LEVEL:
            errors.append(
                f"Level must be between {cls.MIN_LEVEL} and {cls.MAX_LEVEL}, " f"got {stats.level}"
            )
        ...
        if not 8 <= stats.armor_class <= 25:
            logger.warning(f"AC {stats.armor_class} outside typical range [8, 25]")
    ...
    def validate_character(cls, character: Character) -> List[str]:
        ...
        if character.equipment:
            ...
            equipment_list = (
                character.equipment
                if isinstance(character.equipment, list)
                else [character.equipment]
            )
        ...
        if (
            character.stats
            and hasattr(character, "proficiency_bonus")
            and character.proficiency_bonus
        ):
            ...
            if isinstance(value, str):
                ...
                # but still remove obvious HTML/script tags
                    ...
                    if value != value.strip()[:500]:
                        logger.debug(
                            f"Sanitized field {key}: removed potential HTML/script content"
                        )
