...
class SourceOrganizer:
    ...
    RELATIONSHIP_PATTERNS = {
        ...
        "requires": [r"requires (.*)", r"needs (.*)", r"must have (.*)", r"prerequisite: (.*)"],
        "replaces": [r"replaces (.*)", r"supersedes (.*)", r"updates (.*)", r"revision of (.*)"],
        "conflicts_with": [
            r"incompatible with (.*)",
            r"conflicts with (.*)",
    ...
    def categorize_source(
        self, source: Source, content_sample: Optional[str] = None, auto_categorize: bool = True
    ) -> List[SourceCategory]:
        """
        ...
        if source.metadata.categories:
            for cat in source.metadata.categories:
                category = SourceCategory(
                    ...
                    category_type=(
                        cat if isinstance(cat, ContentCategory) else ContentCategory.RULES
                    ),
                    auto_assigned=False,
                    is_primary=True,
                )
        ...
        if auto_categorize:
            ...
            if not content_sample and source.content_chunks:
                ...
                content_sample = " ".join(
                    [chunk.get("content", "")[:500] for chunk in sample_chunks]
                )
    ...
    def detect_relationships(
        self, source: Source, existing_sources: List[Source]
        ...
        for rel_type, patterns in self.RELATIONSHIP_PATTERNS.items():
            for pattern in patterns:
                ...
                if match:
                    ...
                    related_source = self._find_source_by_name(related_name, existing_sources)
    ...
    def organize_by_system(self, sources: List[Source]) -> Dict[str, List[Source]]:
        """
        ...
        for system in organized:
            organized[system].sort(key=lambda s: (s.metadata.source_type.value, s.metadata.title))

        return dict(organized)

    def organize_by_type(self, sources: List[Source]) -> Dict[SourceType, List[Source]]:
        """
    ...
    def organize_by_category(self, sources: List[Source]) -> Dict[ContentCategory, List[Source]]:
        """
    ...
    def find_related_sources(
        self, source_id: str, relationship_type: Optional[str] = None, max_depth: int = 1
    ...
    def build_dependency_tree(
        ...
        for rel in relationships:
            ...
            rel_info = {"id": related.id, "title": related.metadata.title, "strength": rel.strength}
    ...
    def _organize_hierarchy(self, categories: List[SourceCategory]) -> List[SourceCategory]:
        ...
        for category in categories:
            for parent, children in hierarchy.items():
                if category.category_type in children:
                    ...
                    parent_cat = next((c for c in categories if c.category_type == parent), None)
                    if parent_cat:
        ...
        if source.metadata.source_type != SourceType.RULEBOOK:
            for other in same_system:
                if other.metadata.source_type == SourceType.RULEBOOK:
                    ...
                    if (
                        "core" in other.metadata.title.lower()
                        or "player" in other.metadata.title.lower()
                    ):
    ...
    def _find_source_by_name(self, name: str, sources: List[Source]) -> Optional[Source]:
    ...
    def _find_source_by_id(self, source_id: str, sources: List[Source]) -> Optional[Source]:
        """Find a source by ID."""
        for source in sources:
