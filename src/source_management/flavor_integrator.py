...
class FlavorIntegrator:
    ...
    def process_flavor_source(
        self, source: Source, content_chunks: Optional[List[Dict[str, Any]]] = None
    ) -> FlavorSource:
        """
    ...
    def blend_flavor_sources(
        ...
        if tones:
            if conflict_resolution == "blend":
                ...
                blended["tone"] = (
                    tones[0] if conflict_resolution == "priority" else random.choice(tones)
                )
        ...
        for element_type in ["characters", "locations", "events", "quotes"]:
            merged = self._merge_narrative_elements(
                sources, element_type, weights, conflict_resolution
            )
        ...
        if weights:
            ...
            blended["creativity_modifier"] = (
                sum(source.creativity_modifier * weights.get(source.id, 1.0) for source in sources)
                / total_weight
                if total_weight > 0
                else 1.0
            )
        else:
            blended["creativity_modifier"] = sum(s.creativity_modifier for s in sources) / len(
                sources
            )
    ...
    def apply_flavor_to_content(
        self, content: str, flavor_source: FlavorSource, intensity: float = 1.0  # 0.0 to 2.0
    ) -> str:
        """
        ...
        if flavor_source.narrative_style:
            transformed = self._apply_style(transformed, flavor_source.narrative_style, intensity)

        # Apply tone
        if flavor_source.tone:
            transformed = self._apply_tone(transformed, flavor_source.tone, intensity)

        # Inject narrative elements if appropriate
        if intensity > 0.5:
            transformed = self._inject_narrative_elements(
                transformed, flavor_source, min(intensity, 1.0)
            )

        return transformed

    def generate_flavor_context(
        self, query: str, flavor_sources: List[FlavorSource], max_elements: int = 5
    ) -> Dict[str, Any]:
        """
    ...
    def _extract_narrative_elements(
        ...
        elements = {"characters": [], "locations": [], "events": [], "quotes": []}

        seen = {"characters": set(), "locations": set(), "events": set(), "quotes": set()}
    ...
    def _inject_narrative_elements(
        self, content: str, flavor_source: FlavorSource, probability: float
    ) -> str:
