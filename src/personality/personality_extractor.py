...
class PersonalityExtractor:
    ...
    def __init__(self):
        ...
        self.tone_indicators = {
            "authoritative": [
                "must", "shall", "will", "require", "mandate", "command",
                "dictate", "decree", "ordain", "it is written", "the law states"
            ],
            "mysterious": [
                "perhaps", "maybe", "unknown", "hidden", "secret", "enigma",
                "whisper", "shadow", "veil", "obscure", "cryptic", "arcane"
            ],
            "scholarly": [
                "research", "study", "examine", "analyze", "hypothesis",
                "theory", "evidence", "conclude", "observe", "document",
                "according to", "it has been noted"
            ],
            "casual": [
                "gonna", "wanna", "yeah", "nope", "stuff", "thing",
                "basically", "kinda", "sorta", "like", "you know"
            ],
            "formal": [
                "therefore", "thus", "hence", "moreover", "furthermore",
                "consequently", "accordingly", "whereas", "hereby", "thereof"
            ],
            "ominous": [
                "doom", "dread", "fear", "terror", "horror", "nightmare",
                "darkness", "evil", "curse", "forbidden", "unspeakable"
            ],
            "whimsical": [
                "delightful", "charming", "enchanting", "magical", "wonder",
                "sparkle", "dance", "sing", "laugh", "joy", "merry"
            ],
            "military": [
                "mission", "objective", "target", "operation", "tactical",
                "strategic", "command", "protocol", "classified", "secure"
            ],
        }
    ...
    def _detect_perspective(self, text: str) -> Dict[str, Any]:
        ...
        instructional_score = len(re.findall(r'\b(step \d+|first,|then,|finally,)', text)) / len(words) if words else 0
    ...
    def _analyze_style(self, text: str) -> Dict[str, Any]:
        ...
        # Formality score
        formal_words = len(re.findall(r'\b(therefore|thus|hence|moreover|furthermore)\b', text.lower()))
    ...
    def _extract_common_phrases(self, sentences: List[str]) -> List[str]:
        ...
        for phrase, count in bigram_counts.most_common(20):
            if count > 2 and not any(word in ['the', 'a', 'an', 'is', 'are'] for word in phrase.split()):
