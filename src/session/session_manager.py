...
class SessionManager:
    ...
    async def create_session(
        self, campaign_id: str, name: str = None, date: datetime = None
    ) -> Session:
        """
        ...
        try:
            ...
            if not session.combat_rounds:
                ...
                session.current_round = max(
                    (cr.round_number for cr in session.combat_rounds), default=1
                )
            await self._store_session(session)
    ...
    async def set_initiative(
        self, session_id: str, initiative_order: List[Dict[str, Any]]
    ) -> Session:
        """
    ...
    async def add_monster(
        self, session_id: str, monster_data: Dict[str, Any], initiative: int = None
    ) -> Monster:
        """
        ...
        try:
            ...
            monster = (
                Monster.from_dict(monster_data) if isinstance(monster_data, dict) else monster_data
            )
    ...
    async def update_monster_hp(self, session_id: str, monster_id: str, new_hp: int) -> Monster:
        """
    ...
    async def get_active_sessions(self) -> List[Session]:
        ...
        try:
            ...
            results = collection.get(
                where={"status": SessionStatus.ACTIVE.value}, include=["documents", "metadatas"]
            )
    ...
    async def _store_session(self, session: Session):
        ...
        try:
            ...
            # Store session
            collection.upsert(
                ids=[session.id], documents=[json.dumps(session.to_dict())], metadatas=[metadata]
            )
