...
class TestHybridSearchEngine:
    ...
    def mock_db(self):
        ...
        db.search = AsyncMock(return_value=[
            {"id": "1", "content": "Test content", "metadata": {}, "distance": 0.1}
        ])
        return db
    ...
    async def test_keyword_search(self, mock_db):
        ...
        with patch.object(engine, '_keyword_search', return_value=[
            {"id": "2", "content": "Keyword match", "score": 0.9}
        ]):
    ...
    async def test_hybrid_search_combination(self, mock_db):
        ...
        semantic_results = [
            {"id": "1", "content": "Semantic match", "score": 0.8}
        ]
        keyword_results = [
            {"id": "2", "content": "Keyword match", "score": 0.7}
        ]
...
class TestSearchService:
    ...
    async def test_search_workflow(self, search_service):
        ...
        search_service.query_processor.expand_query.return_value = ["normalized", "query", "expanded"]
...
class TestSearchIntegration:
    ...
    async def test_cross_reference_search(self):
        ...
        # Mock campaign data
        with patch.object(service, '_get_campaign_context', return_value={
    ...
    async def test_multi_collection_search(self):
        ...
        results = await service.search_multiple_collections(
            "test",
            ["rulebooks", "flavor_sources"]
        )
