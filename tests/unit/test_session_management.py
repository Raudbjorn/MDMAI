...
class TestSessionModels:
    ...
    def test_initiative_entry(self):
        ...
        entry = InitiativeEntry(
            id="init_1", name="Gandalf", initiative=18, is_player=True, hp_current=50, hp_max=50
        )
    ...
    def test_monster_status_transitions(self):
        ...
        monster = Monster(id="monster_1", name="Dragon", hp_current=200, hp_max=200, ac=18)
...
class TestSessionManager:
    ...
    async def test_create_session(self, session_manager, mock_db):
        ...
        session_id = await session_manager.create_session(
            campaign_id="campaign_1", title="Session 1: The Beginning", session_number=1
        )
    ...
    async def test_add_initiative_entry(self, session_manager, mock_db):
        ...
        entry = await session_manager.add_to_initiative(
            session_id="session_1", name="Gandalf", initiative=18, is_player=True, hp=50
        )
    ...
    async def test_add_monster(self, session_manager, mock_db):
        ...
        monster_id = await session_manager.add_monster(
            session_id="session_1", name="Goblin", hp=7, ac=15, initiative=12
        )
    ...
    async def test_update_monster_hp(self, session_manager, mock_db):
        ...
        updated = await session_manager.update_monster_hp(
            session_id="session_1", monster_id="monster_1", damage=3
        )
    ...
    async def test_heal_monster(self, session_manager, mock_db):
        ...
        updated = await session_manager.update_monster_hp(
            session_id="session_1", monster_id="monster_1", damage=-4  # Negative damage = healing
        )
    ...
    async def test_combat_round_tracking(self, session_manager, mock_db):
        ...
        # Start combat
        await session_manager.start_combat(session_id="session_1", initial_round=1)

        # Advance round
        await session_manager.advance_round(
            session_id="session_1", actions=["Gandalf casts Fireball"], damage_dealt={"Goblin": 28}
        )
    ...
    async def test_end_session(self, session_manager, mock_db):
        ...
        summary = await session_manager.end_session(
            session_id="session_1", summary="The party defeated the goblin ambush"
        )
