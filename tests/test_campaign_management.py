...
from src.campaign.models import Campaign, CampaignData, NPCData, LocationData, PlotPoint
...
class TestCampaignManager:
    ...
    async def test_rollback_campaign(self, campaign_manager, mock_db):
        ...
        result = await campaign_manager.rollback_to_version(
            "test_campaign",
            "version_1"
        )
...
class TestRulebookLinker:
    ...
    async def test_auto_link_campaign(self, rulebook_linker, mock_db):
        ...
        campaign_data = {
            "npcs": [
                {"id": "npc_1", "class": "wizard"},
                {"id": "npc_2", "class": "fighter"}
            ],
            "locations": [
                {"id": "loc_1", "type": "city"}
            ]
...
class TestCampaignIntegration:
    ...
    async def test_campaign_workflow(self):
        ...
        # Add NPCs
        npc1 = await manager.add_npc(campaign_id, {
            "name": "Villain",
            "role": "antagonist"
        })
        
        # Add locations
        loc1 = await manager.add_location(campaign_id, {
            "name": "Dark Tower",
            "type": "dungeon"
        })
        
        # Add plot points
        plot1 = await manager.add_plot_point(campaign_id, {
            "title": "Final Battle",
            "npcs": [npc1],
            "location": loc1
        })
    ...
    async def test_campaign_search_integration(self):
        ...
        results = await manager.search_campaign_content(
            "test",
            "dragon"
        )
