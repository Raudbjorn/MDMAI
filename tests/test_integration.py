    ...
    async def test_complete_game_session_workflow(self):
        ...
        with patch("src.core.database.ChromaDBManager") as MockDB:
            ...
            campaign_id = await campaign_mgr.create_campaign(
                name="Test Campaign", system="D&D 5e", description="Integration test campaign"
            )
            assert campaign_id is not None

            # Step 2: Add NPCs to campaign
            npc_id = await campaign_mgr.add_npc(
                campaign_id, {"name": "Dragon", "role": "boss", "hp": 200}
            )
            assert npc_id is not None

            # Step 3: Create a session
            session_id = await session_mgr.create_session(
                campaign_id=campaign_id, title="Boss Battle", session_number=10
            )
            assert session_id is not None

            # Step 4: Add combatants to initiative
            await session_mgr.add_to_initiative(session_id, "Fighter", 15, True, 50)
            await session_mgr.add_monster(session_id, "Dragon", 200, 18, 10)

            # Step 5: Run combat rounds
            await session_mgr.start_combat(session_id)
            await session_mgr.advance_round(session_id, actions=["Fighter attacks Dragon"])

            # Step 6: Add session notes
            await session_mgr.add_note(session_id, "Epic battle with the dragon begins!")

            # Step 7: End session
            await session_mgr.end_session(session_id, "Dragon defeated, party victorious!")
                ...
                with patch.object(pipeline.parser, "extract_text_from_pdf") as mock_extract:
                    ...
                    with patch.object(pipeline, "_is_duplicate", return_value=False):
                        with patch.object(pipeline.chunker, "chunk_document") as mock_chunk:
                            ...
                            with patch.object(
                                pipeline.embedding_generator, "generate_embeddings"
                            ) as mock_embed:
                                mock_embed.return_value = [[0.1, 0.2, 0.3]]

                                result = await pipeline.process_pdf(
                                    pdf_path=tmp_path, rulebook_name="Test Book", system="D&D 5e"
                                )
    ...
    async def test_character_generation_with_rulebook_integration(self):
        ...
        with patch("src.core.database.ChromaDBManager") as MockDB:
            ...
            character = generator.generate_character(
                name="Test Hero", race="human", character_class="fighter", level=5
            )
    ...
    async def test_campaign_with_personality_system(self):
        ...
        with patch("src.personality.personality_manager.PersonalityManager") as MockPM:
            ...
            backstory = generator.generate_backstory(
                character_class="rogue", race="halfling", system="Blades in the Dark"
            )
    ...
    async def test_search_with_campaign_context(self):
        ...
        with patch("src.core.database.ChromaDBManager") as MockDB:
            ...
            results = await service.search_with_context(
                query="wizard spells", campaign_id="campaign_1"
            )
    ...
    async def test_session_with_monster_lookup(self):
        ...
        with patch("src.core.database.ChromaDBManager") as MockDB:
            ...
            monster = await manager.add_monster_from_rulebook(
                session_id="session_1", monster_name="Goblin"
            )
    ...
    async def test_parallel_processing_integration(self):
        ...
        try:
            ...
            with patch.object(processor, "_process_pdf_task") as mock_pdf:
                ...
                pdf_task = await processor.submit_task("pdf_processing", {"pdf_path": "test.pdf"})
...
class TestPerformanceIntegration:
    ...
    async def test_cache_with_search(self):
        ...
        from src.performance.cache_system import CachePolicy, CacheSystem
        from src.search.search_service import SearchService

        # Create cache
        cache = CacheSystem(name="test_cache", max_size=10, ttl_seconds=60, policy=CachePolicy.LRU)

        mock_db = Mock()
        mock_db.search = AsyncMock(return_value=[{"id": "1", "content": "Cached result"}])
    ...
    async def test_database_optimization(self):
        ...
        optimizer.track_query_performance(
            query="test", collection="test", execution_time=0.1, result_count=5
        )
            ...
            with patch.object(pipeline, "_is_duplicate", return_value=False):
                with patch.object(pipeline.chunker, "chunk_document") as mock_chunk:
                    ...
                    with patch.object(pipeline.embedding_generator, "generate_embeddings"):
                        result = await pipeline.process_pdf(
                            pdf_path="test.pdf", rulebook_name="Test", system="D&D 5e"
                        )
...
class TestDataConsistency:
    ...
    async def test_campaign_version_consistency(self):
        ...
        version_id = await manager.create_version_snapshot("campaign_1", "Before major event")
    ...
    async def test_cross_reference_integrity(self):
        ...
        session_id = await session_mgr.create_session(campaign_id=campaign_id, title="Test Session")
