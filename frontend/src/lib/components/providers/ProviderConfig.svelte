<script lang="ts">
  ...
  const providerNames: Record<ProviderType, string> = {
    ...
    [ProviderType.FISH_AUDIO]: 'Fish Audio',
    [ProviderType.OLLAMA_TTS]: 'Ollama TTS',
  };
  ...
  const providerIcons: Record<ProviderType, string> = {
    ...
    [ProviderType.OPENAI]: '🧠',
    [ProviderType.GOOGLE]: '🌐',
    [ProviderType.ELEVENLABS]: '🗣️',
    [ProviderType.FISH_AUDIO]: '🐟',
    [ProviderType.OLLAMA_TTS]: '🦙',
  };
      ...
      <div>
        ...
        {#if health}
          <div class="flex items-center gap-2 mt-1">
            <span
              class="status-badge status-{statusConfig[health.status].color}"
            >
              {statusConfig[health.status].icon}
              {statusConfig[health.status].text}
            </span>
    ...
    <div class="config-details mt-4">
      {#if !isEditing}
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
            <p class="text-gray-900 dark:text-gray-100">
              {config.rate_limit_tpm.toLocaleString()} tokens/min
            </p>
          </div>
          ...
          <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <div class="grid grid-cols-3 gap-4 text-sm">
              ...
              <div>
                ...
                >
                <span class="ml-1 font-medium"
                  >${stats.total_cost.toFixed(2)}</span
                >
              </div>
            </div>
          </div>
      ...
      {:else}
        ...
        <div class="space-y-4">
          ...
          {#if providerType === ProviderType.ELEVENLABS || providerType === ProviderType.FISH_AUDIO || providerType === ProviderType.OLLAMA_TTS}
            <div class="grid grid-cols-2 gap-4 pt-2">
              <div>
                ...
                <input
                  type="text"
                  bind:value={editConfig.voice_id}
              ...
              <div>
                <label
                  class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >
                  Cost Limit Override (USD)
                </label>
                <input
                  type="number"
                  bind:value={editConfig.cost_limit_usd}
                  step="0.01"
                  min="0"
                  placeholder="Max cost per request"
                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md
									       bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                />
              </div>
          ...
          await providerStore.configureProvider(
            {
              ...
              providerType === ProviderType.FISH_AUDIO ||
              providerType === ProviderType.OLLAMA_TTS
                ? {
                    voice_id: 'default',
                    cost_limit_usd: 1.0,
                  }
