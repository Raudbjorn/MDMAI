<script lang="ts">
	...
	const apiKeyPatterns: Record<ProviderType, RegExp> = {
		...
		[ProviderType.ELEVENLABS]: /^[a-f0-9]{32}$/,
		[ProviderType.FISH_AUDIO]: /^[a-f0-9]{32}$/, // Assuming hex format, adjust if known
		[ProviderType.OLLAMA_TTS]: /.*/ // No strict pattern for local/custom currently
	};
	...
	const docUrls: Record<ProviderType, string> = {
		...
		[ProviderType.ELEVENLABS]: 'https://elevenlabs.io/app/speech-synthesis',
		[ProviderType.FISH_AUDIO]: 'https://fish.audio/developer',
		[ProviderType.OLLAMA_TTS]: 'https://github.com/ollama/ollama'
	};
